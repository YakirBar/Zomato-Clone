                ...
                script {
                    ...
                    if (issueKey) {
                        ...
                        if (transitionId != null) {
                            def response = sh(script: "curl -u admin:password -X POST -H 'Content-Type: application/json' -d '{\"transition\": {\"id\": \"${transitionId}\"}}' http://localhost:8090/rest/api/2/issue/${issueKey}/transitions", returnStdout: true)
...
def getTransitionId(issueKey, statusName) {
    def response = sh(script: "curl -u admin:password -X GET -H 'Content-Type: application/json' http://localhost:8090/rest/api/2/issue/${issueKey}/transitions", returnStdout: true).trim()
